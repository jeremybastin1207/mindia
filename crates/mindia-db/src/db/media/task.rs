...
impl TaskRepository {
    ...
    /// Reap tasks stuck in 'running' for longer than their timeout + grace period.
    /// Resets them to 'pending' for retry (if retries left), or 'failed' if max retries exceeded.
    ...
    pub async fn reap_stale_running_tasks(&self, grace_period_secs: i64) -> Result<u64> {
        ...
        let reset_result = sqlx::query(
            ...
            WHERE status = 'running'
              AND started_at < NOW() - (COALESCE(timeout_seconds, 3600) + $1) * INTERVAL '1 second'
        ...
        let reset_count = reset_result.rows_affected();

        // Mark as failed where retry_count >= max_retries
        ...
        .bind(serde_json::json!({
            "error": "Task abandoned (stale runner)",
            "reason": "Task was running longer than timeout + grace period; instance may have crashed"
        }))
        .execute(&self.pool)
        .await
        .context("Failed to mark stale running tasks as failed")?;

        let failed_count = failed_result.rows_affected();
        let total = reset_count + failed_count;
