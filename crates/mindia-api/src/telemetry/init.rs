...
pub fn init_telemetry(
    ...
    #[cfg(feature = "observability-opentelemetry")]
    {
        ...
        let instance_id = env::var("OTEL_SERVICE_INSTANCE_ID")
        ...
        let otlp_reader = PeriodicReader::builder(otlp_exporter, opentelemetry_sdk::runtime::Tokio)
            .with_interval(Duration::from_secs(_metrics_interval_secs))
            .build();
        ...
        // Console: compact format (message string). OTLP layer gets full structured fields.
        let console_fmt = tracing_subscriber::fmt::layer().event_format(
            Format::default()
                .compact()
                .with_target(false)
