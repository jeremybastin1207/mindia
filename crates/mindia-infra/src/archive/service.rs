...
pub async fn create_zip_archive(
    ...
    let mut buffer = Vec::new();
    {
        ...
        for (_media_id, storage_key, original_filename) in media_items {
            ...
            zip.start_file(&original_filename, options)
                .with_context(|| format!("Failed to add file to ZIP: {}", original_filename))?;
            zip.write_all(&file_data).with_context(|| {
                format!("Failed to write file data to ZIP: {}", original_filename)
            })?;
        }
...
pub async fn create_tar_archive(
    ...
    let mut buffer = Vec::new();
    {
        ...
        for (_media_id, storage_key, original_filename) in media_items {
            ...
            // Add file to archive
            tar.append_data(&mut header, &original_filename, &*file_data)
                .with_context(|| format!("Failed to add file to TAR: {}", original_filename))?;
        }
