...
use mindia_core::{AppError, CapacityGate, Config};
...
#[async_trait]
impl CapacityGate for CapacityChecker {
    async fn can_accept_task(&self) -> bool {
        if self.check_cpu_usage_async().await.is_err() {
            return false;
        }
        if self.check_memory_async(0).await.is_err() {
            return false;
        }
        true
    }
